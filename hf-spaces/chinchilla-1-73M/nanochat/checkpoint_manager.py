...
def save_checkpoint(
    ...
    # Sync to disk so Modal's FUSE volume picks up the writes promptly.
    # Without this, a container killed by timeout may lose uncommitted checkpoint data.
    import os as _os
...
def load_model_from_dir(checkpoints_dir, device, phase, model_tag=None, step=None):
    if model_tag is None:
        # Check if there are model files directly in checkpoints_dir (flat structure)
        model_files = glob.glob(os.path.join(checkpoints_dir, "model_*.pt"))
        if model_files:
